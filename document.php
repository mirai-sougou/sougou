...
try {
    ...
    if (isset($_POST['back']) && $_POST['back']) {
        ...
        if (!$_POST["birth"]) {
            $errmessage['birth'] = "生年月日を入力してください";
        }
        if (!$_POST["postalCode"]) {
        ...
        if (!$_POST['token'] || !$_SESSION['token'] || !$_SESSION['email']) {
            ...
            $stmt = $pdo->prepare("INSERT INTO document (fullname, email,message,tel,grade,gakkou,kana,gender,birth,postalCode,prefecture,city,address1,address2) VALUES (?, ?, ?,?,?,?,?,?,?,?,?,?,?,?)");
            $stmt->execute(
                array(
                    $_SESSION['fullname'],
                    $_SESSION['email'],
        ...
        <main>
                ...
                <form action="./document.php" method="post">
                    ...
                    <div class="form-group row justify-content-center">
                        <label for="datepicker" class="col-md-3 col-form-label text-md-right">
                            生年月日<span class="text-danger" style="margin-left: 5px;">必須</span></label>
                        <div class="col-md-6">
                            <input type="text" id="datepicker" name="birth" class="form-control"
                                value="<?php echo $_SESSION['birth'] ?>">
                        </div>
                    </div>
                    <br>
                    <?php
                    if ($errmessage && isset($errmessage['birth'])) {
                        echo '<div class="alert alert-danger" role="alert">';
                        echo $errmessage['birth'];
                        echo '</div>';
                    }
                    ?>
            ...
            <?php } else if ($mode == 'confirm') { ?>
                    ...
                    <form action="./document.php" method="post">
                        ...
                        <div class="form-group row justify-content-center">
                            <label for="birth" class="col-md-3 col-form-label text-md-right">生年月日<span class="text-danger"
                                    style="margin-left: 5px;">必須</span></label>
                            <div class="col-md-6">
